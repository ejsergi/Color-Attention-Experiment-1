function [TESTMoni,info,legendIm,infoSur] = CreatePatt_Hue...
    (meansvalues,L_Target,Deviation,HueRange,HueDeviation,SL_Target,SDeviation,sHueRange)
%%%%%
% meansvalues = Mean between stimuli and background 
...
% HueRange = (CHANGING VIRABLE) Range of stimuly hue
...
for i=1:length(HueRange);

Sel = logical((Li<=L_Target+Deviation).*(Li>=L_Target-Deviation)...
    .*(Hue<HueRange(i)+HueDeviation).*(Hue>HueRange(i)-HueDeviation));
